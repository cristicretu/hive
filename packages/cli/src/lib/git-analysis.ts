...
export interface FileOverlap {
  file: string;
  tasks: string[];
  changeType: 'same-file-modified' | 'structural-conflict' | 'dependency-change';
}

/**
 * Get detailed diff analysis for a task
 */
export async function getTaskDiffAnalysis(task: Task): Promise<TaskDiffAnalysis> {
 ...
 */
function determineChangeType(diff: string, filename: string): 'added' | 'modified' | 'deleted' {
...
/**
 * Find files that are modified by multiple tasks
 */
export async function getOverlappingFiles(tasks: Task[]): Promise<FileOverlap[]> {
  const fileMap = new Map<string, Set<string>>();

  // Build map of file -> set of task slugs
  for (const task of tasks) {
    const analysis = await getTaskDiffAnalysis(task);
    
    for (const file of analysis.files) {
      if (!fileMap.has(file.path)) {
        fileMap.set(file.path, new Set());
      }
      fileMap.get(file.path)!.add(task.slug);
    }
  }
  ...
  for (const [file, taskSet] of fileMap.entries()) {
    if (taskSet.size > 1) {
      overlaps.push({
        ...
        changeType: categorizeOverlap(file, taskSet.size),
      });
    }
  }
 ...
 */
function categorizeOverlap(filename: string, taskCount: number): FileOverlap['changeType'] {
  if (filename === 'package.json' || filename === 'package-lock.json' || filename === 'bun.lock') {
  ...
  return 'same-file-modified';
}

/**
 * Get file contents from a specific task worktree
 */
export async function getFileFromTask(task: Task, filePath: string): Promise<string | null> {
 ...
 */
export async function getFileFromMain(filePath: string): Promise<string | null> {
  const fs = await import('fs/promises');
  const path = await import('path');
...
export async function analyzeAllTasks(tasks: Task[]): Promise<{
  ...
  const overlaps = await getOverlappingFiles(tasks);
