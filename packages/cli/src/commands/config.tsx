...
import { getConfig, setConfig, type AIConfig } from '../lib/config.js';
	...
	if (action === 'set') {
		...
		try {
			...
			if (keys.length === 2 && keys[0] === 'ai') {
				const aiConfig: AIConfig = { ...DEFAULT_AI_CONFIG, ...config.ai };
				const aiKey = keys[1] as keyof typeof aiConfig;

				if (aiKey === 'provider' || aiKey === 'model' || aiKey === 'apiKey') {
					if (aiConfig.enabled !== true) {
						throw new Error('AI is disabled. Enable it first: hive config set ai.enabled true');
					}
				}
